...
from .models import LoanRequest, Account, Tarjeta, Cliente, Cuenta, Sucursal, Marca, Prestamo, Empleado, SolicitudPrestamo
...
from rest_framework.response import Response
from .serializers import AccountSerializer, LoanRequestSerializer, MarcaSerializer, SucursalSerializer, ClienteSerializer, CuentaSerializer, PrestamoSerializer, EmpleadoSerializer, TarjetaSerializer, RegistroSerializer, SolicitudPrestamoSerializer, UserRegisterSerializer
from rest_framework_simplejwt.tokens import RefreshToken
from rest_framework.views import APIView
...
class SolicitarPrestamoView(APIView):
    ...
    def post(self, request):
        # Validar si el usuario ya tiene 3 solicitudes
        solicitudes_count = SolicitudPrestamo.objects.filter(user=request.user).count()
        if solicitudes_count >= 3:
            return Response({"error": "Solo puedes realizar hasta 3 solicitudes de préstamo."}, status=status.HTTP_400_BAD_REQUEST)

        # Deserializar los datos del cuerpo de la solicitud
        serializer = SolicitudPrestamoSerializer(data=request.data)
        if serializer.is_valid():
            # Guardar la solicitud con el usuario autenticado
            solicitud = serializer.save(user=request.user)
            return Response({
                "success": "Solicitud de préstamo enviada exitosamente.",
