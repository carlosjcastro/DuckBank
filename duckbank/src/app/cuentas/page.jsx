...
import { FaDollarSign, FaEye, FaEyeSlash, FaChevronUp, FaChevronDown, FaEllipsisH } from 'react-icons/fa';
import { LuBanknote } from 'react-icons/lu';
import Transferencias from './transferencias/page';
import Servicios from '../servicios/page';
import Link from 'next/link';
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Snackbar, Menu, MenuItem } from '@mui/material';
...
const getIconComponent = (iconKey) => ICONS[iconKey] || <FaDollarSign size={24} className="inline mr-2" />;
...
export default function Cuentas() {
    ...
    useEffect(() => {
      ...
      const defaultAccounts = [
        { id: 1, name: 'Cuenta Principal', balance: 285547, currency: 'ARS', icon: 'LuBanknote' },
        { id: 2, name: 'Caja de Ahorros', balance: 899504, currency: 'ARS', icon: 'LuBanknote' },
        { id: 3, name: 'Cuenta Dólares', balance: 28039, currency: 'USD', icon: 'FaDollarSign' },
      ];
    ...
    const handleSearch = (e) => {
      ...
      setSearchTerm(term);
      setFilteredAccounts(
        externalAccounts.filter(account => account.name.toLowerCase().includes(term))
      );
    };
      ...
      if (dialogType === 'modify') {
        setAccounts((prevAccounts) =>
          prevAccounts.map((account) =>
            account.name === newAccountName ? { ...account, name: newAccountName } : account
          )
        );
        ...
        setExternalAccounts((prevExternalAccounts) => [...prevExternalAccounts, newExternalAccount]);
    ...
    const handleConfirmDeleteClose = (confirm) => {
      if (confirm && selectedAccount) {
        ...
        localStorage.setItem('externalAccounts', JSON.stringify(updatedExternalAccounts));
  ...
  return (
    <div className="container mx-auto p-4 bg-[#F5F5F5]">
      ...
      <div className="flex justify-between items-center p-4">
        ...
        <h2 className="text-xl font-semibold text-[#143D60]">Mis Cuentas</h2>
        <Button
          onClick={toggleBalancesVisibility}
          className="text-[#143D60] focus:outline-none"
        >
      ...
      <div className="flex flex-col items-center">
        <div className="flex flex-row gap-4 flex-wrap justify-center">
          {accounts.map((account) => (
            <div key={account.id} className="flex flex-col items-center p-6 border border-[#BCBDC0] rounded-lg bg-white shadow-md transition duration-300 hover:bg-[#F5F5F5]">
              <div className="flex items-center mb-4">
                {getIconComponent(account.icon)}
                <div className="ml-4">
                  <span className="text-lg font-medium text-[#143D60]">{account.name}</span>
                  {showBalances && (
                    <div className="mt-1 text-xl font-semibold text-[#143D60]">
      ...
      <div className="p-4">
        <div className="flex items-center">
          <h2 className="text-xl font-semibold text-[#143D60]">Cuentas Externas</h2>
          <button onClick={toggleExternalAccountsVisibility} className="ml-2 text-[#143D60]">
            <svg className={`w-6 h-6 ${showExternalAccounts ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        {showExternalAccounts && (
          <div className="mt-4">
            <input
              ...
              className="border border-[#BCBDC0] rounded p-2 w-full mb-4 bg-white text-[#143D60]" />
            {filteredAccounts.map((account) => (
              <div key={account.id} className="relative p-4 border border-[#BCBDC0] rounded-lg bg-white mb-2">
                {getIconComponent(account.icon)}
      ...
      >
        <MenuItem onClick={() => handleMenuOptionClick('modify')}>Modificar nombre</MenuItem>
        <MenuItem onClick={() => handleMenuOptionClick('delete')}>Eliminar cuenta</MenuItem>
      </Menu>
      ...
      <Dialog open={confirmDeleteOpen} onClose={() => handleConfirmDeleteClose(false)}>
        <DialogTitle>Confirmar eliminación</DialogTitle>
        <DialogContent>
          <p>¿Estás seguro de que deseas eliminar la cuenta {selectedAccount?.name}?</p>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => handleConfirmDeleteClose(false)} color="primary">
            Cancelar
          </Button>
          <Button onClick={() => handleConfirmDeleteClose(true)} color="secondary">
            Eliminar
          </Button>
        </DialogActions>
      </Dialog>
  ...
  <Alert onClose={() => setSnackbarOpen(false)} severity="success">
    {snackbarMessage}
      ...
      <Link href="/servicios" passHref>
        <Button variant="contained" className="fixed bottom-4 right-4 bg-[#9DAB70] hover:bg-[#7A8B50]">
          Ir a Pagos
        </Button>
      </Link>
    </div>
